...
- **`rag`:** course/syllabus/`key_concepts`/lectures/OCW **plus the exercise & practice prompts** are chunked + embedded offline (`rag.ingest`), and only the chunks most relevant to the student's message are retrieved and folded into the tutor's **system message** — appended after the static cacheable prompt (not prepended to the student's turn), so prompt caching still hits — far cheaper than dumping every transcript. Solutions are excluded from the index (they're paired directly, above); figures/metadata are excluded too. See [`rag/README.md`](rag/README.md).
...
│   └── prompts/             # tutor_01.txt .. tutor_06.txt (versioned prompts; tutor_05 is the deployed default, both apps locked to it; tutor_06 is the experimental citations/anti-leak variant)
│
├── internal_testing/
...
- **AskTIM (`main_ui/`)** is feature-complete through Step 10 (image uploads) — Postgres persistence, username + password identity, cross-browser history, SSE-streamed replies, **student PNG/JPEG uploads plus non-image attachments** (CSV/TSV/XLSX/PDF/DOCX/TXT, extracted to text and persisted across turns), tutor replies rendered as sanitized markdown + **KaTeX** math (locked to the deployed `tutor_05`), and per-message **thumbs up/down** ratings on each tutor reply (a `messages.rating` column; the older conversation-level 1–5 star feedback toast is kept but dormant) — and is **live on Railway at <https://asktim.up.railway.app/>** (containerized, migrations run on boot). Steps 11–12 (multi-iframe test host, formal test suite) remain.
