...
- **Solution keys are NOT in tutor context** (withdrawn 2026-07-21) — the paired "correct answer & worked solution" used to be injected as a tutor-only reference, but a simulated run leaked a verbatim key value to a student who never asked for it. The tutor can no longer check a student's answer against the key; see [`ui_core/tutor_bridge.py`](../ui_core/tutor_bridge.py) `build_assignment_text`
