...
def main() -> int:
    ...
    # --- current_degraded lazy expiry: within cooldown stays down, past it recovers ---
    with Session(engine) as s3:
        ...
        s3.commit()
        # 30s later, cooldown 90s -> still degraded
        ...
        # 200s after degraded_since (=now+4s) -> lazily recovered
        recovered = svc.current_degraded(s3, cooldown_seconds=90, now=now + timedelta(seconds=210))
        s3.commit()
        ok &= _check("past cooldown: current_degraded False (lazy expiry)", recovered is False)
