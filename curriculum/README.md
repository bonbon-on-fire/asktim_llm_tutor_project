...
curriculum/
  <course_name>/
    ...
    pinned/                          # optional — always-pinned reference docs (folded into context; NOT retrieved)
      optimization_debugging_flowchart.txt
      ...
...
- `pinned/` (optional) holds **always-pinned reference docs** — every `pinned/*.txt` is folded directly into the tutor's context (like `course.txt`/`syllabus.txt`) in `full_context` and `rag` modes, and is **excluded from the RAG index** (never retrieved). Each file should carry its own title as its first line; they're read (sorted, stripped, blank-line-joined) by `read_pinned_context()` in [`utils/curriculum.py`](../utils/curriculum.py). Use it for course reference material the tutor should always have on hand — e.g. `supply_chain_design/pinned/optimization_debugging_flowchart.txt`, the course's official Solver debugging flow chart.
