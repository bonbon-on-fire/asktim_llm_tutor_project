...
- **Tutor** — two controls. The **prompt** dropdown lists every built-in prompt
  for visibility but is **locked to `tutor_09`** (disabled; the routes ignore any
...
- `"tutor"` — **ignored**; the sandbox is locked to `tutor_09` server-side, so any supplied value is discarded
