...
- **Hard-down provider alerting (Tier 2)** — a second, narrower signal layered on top of the passive counter above: [`services/provider_alerts.py`](services/provider_alerts.py) inspects each failed tutor turn and fires only when the cause is a **human-actionable** provider condition that won't self-heal — credit/billing exhaustion, invalid auth, or a permission denial (transient rate-limit/timeout/5xx errors are already retried in `tutor/run_tutor.py` and are deliberately ignored here). Classification is duck-typed on the exception class/message, so it works for both the Anthropic and OpenAI SDKs without importing either. It alerts on two channels: a `TUTOR_PROVIDER_DOWN` CRITICAL log line (always, so log-based alerting can key on the marker) and an optional webhook POST (`ALERT_WEBHOOK_URL`), debounced per worker to at most one every `PROVIDER_ALERT_MIN_INTERVAL_SECONDS` (default 300). Best-effort and non-blocking — it never raises inside a student's SSE stream.
...
| `ALERT_WEBHOOK_URL` | *(unset)* | Webhook to POST hard-down provider alerts to (`services/provider_alerts.py`); unset disables the webhook (the `TUTOR_PROVIDER_DOWN` CRITICAL log line still fires). |
| `PROVIDER_ALERT_MIN_INTERVAL_SECONDS` | `300` | Per-worker debounce for the provider-alert webhook, so a burst of failing turns doesn't spam the channel. |
...
main_ui/
  ...
  services/
    ...
    provider_alerts.py    # Tier-2 hard-down provider alerting (credit/auth/permission) — log marker + optional webhook
