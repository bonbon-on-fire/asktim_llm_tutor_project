...
- **Judge prompt**: Available from `eval/tutor_judge/prompts/judge_*.txt` (required)
- **Judge rubric**: Available from `eval/tutor_judge/rubrics/rubric_*.md` (required)

**Command-line mode:**
```powershell
# Grade with GPT (reads *_raw/, writes *_judge/)
python -m internal_testing.run_transcript_judge --provider gpt --prompt judge_08 --rubric rubric_08

# Grade with Claude (reads *_raw/, writes *_judge/)
python -m internal_testing.run_transcript_judge --provider claude --prompt judge_08 --rubric rubric_08

# Read from *_mini/ instead of *_raw/ (--yes skips confirmation)
python -m internal_testing.run_transcript_judge --provider claude --prompt judge_08 --rubric rubric_08 \
  --source-suffix mini --yes
```

The script discovers all transcripts matching `*_{source-suffix}/transcript_*.json`, copies each to the provider-agnostic `*_judge/` folder, then applies judging in-place.
...
Judged transcripts are saved to a provider-agnostic `*_judge/` folder (the judge provider/prompt/rubric used are recorded inside each graded file's `grade` object, not in the folder name):

- `transcripts/chaotic/chaotic_judge/`
- `transcripts/clueless/clueless_judge/`
- `transcripts/cooperative/cooperative_judge/`
